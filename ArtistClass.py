...
class Artist(threading.Thread):
    def __init__(self, artist_id, skill, MusicStyle, locationList, clock, prob_drink=0.3, prob_drugs=0.1):
        super().__init__()
        self.id = artist_id
        self.skill = skill
    ...
    def run(self):
        while self.clock.active:
            while self.active:
                ...
                while self.health > 0 and self.clock.getTime()<self.clock.dayLength:

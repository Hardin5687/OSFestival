...
def main():
    ...
    locationList['dealers']=[DrugDealer(locationList['stages'][0], name='Johnny')]
    t=SecurityGuard(locationList['stages'], locationList, name='Filo')
    t.start()
    clock = Clock()
    for i in range(10):
        time.sleep(0.1)
        t = Spectator(i+1, personalities[random.choice(list(personalities.keys()))], locationList, locationList['exits'][0], clock)
        t.start()
    #Make locations
    #Connect locations
    #Make clock
